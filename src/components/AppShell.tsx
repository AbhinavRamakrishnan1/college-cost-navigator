    ...
    <div className="min-h-screen" onKeyDown={event=>{if(event.key==='Escape'&&open){setOpen(false);menuButton.current?.focus()}}}>
      ...
      <footer className="mt-20 border-t border-line bg-white py-10">
        <div className="page-wrap grid gap-6 text-sm text-ink-700 md:grid-cols-[1fr_auto]">
          ...
          <div className="flex flex-wrap gap-x-5 gap-y-2"><Link to="/about">About</Link><Link to="/contact">Contact</Link><Link to="/aid-offer-decoder">Aid offer decoder</Link><Link to="/privacy">Privacy</Link><Link to="/methodology">Methodology</Link><Link to="/policy-changes">Policy changes</Link></div>
        </div>
      </footer>
    </div>
  )
