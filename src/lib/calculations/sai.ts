...
export function calculateDependentSai(input: DependentSaiInputs): SaiResult {
  ...
  const sAllow = saiWhole(D(n(input.studentIncome.incomeTaxPaid)).plus(sHi).plus(sOasdi).plus(11770).plus(paai < 0 ? Math.abs(paai) : 0))
