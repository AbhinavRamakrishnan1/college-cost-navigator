...
export function allocateFederalLoans(input: LoanAllocationInput) {
  ...
  const ledgerInputs = [
    ...(requestedSub ? [{ borrowerId: value.directElection.borrowerId, borrowerRole: 'student' as const, studentBeneficiaryId: value.studentBeneficiaryId, loanType: 'direct_subsidized' as const, academicYear: value.academicYear, schedule: value.directElection.subsidizedSchedule }] : []),
    ...(requestedUnsub ? [{ borrowerId: value.directElection.borrowerId, borrowerRole: 'student' as const, studentBeneficiaryId: value.studentBeneficiaryId, loanType: 'direct_unsubsidized' as const, academicYear: value.academicYear, schedule: value.directElection.unsubsidizedSchedule }] : []),
    ...value.parentPlusElections.filter((item) => item.grossPrincipalCents > 0).map((item) => ({ borrowerId: item.borrowerId, borrowerRole: 'parent' as const, studentBeneficiaryId: value.studentBeneficiaryId, loanType: 'parent_plus' as const, academicYear: value.academicYear, schedule: item.schedule })),
  ]
