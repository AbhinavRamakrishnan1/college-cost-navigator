...
export class NavigatorDatabase extends Dexie {
  ...
  constructor(name = DATABASE_NAME) {
    ...
    this.version(DATABASE_VERSION).stores({profiles:'&id, updatedAt',metadata:'&id',savedSchools:'&unitId, addedAt'}).upgrade(async(transaction)=>{await transaction.table('metadata').put(STORAGE_METADATA)})
  }
}
