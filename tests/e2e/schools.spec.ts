...
test('finds, opens, saves, reloads, and removes a school without household data requests',async({page})=>{
  ...
  await page.goto('/app/schools');await page.getByLabel('Institution name').fill('Ohio State University-Main Campus');await expect(page.getByText('1 result')).toBeVisible()
  await page.getByRole('button',{name:'View details'}).click();await expect(page.getByRole('heading',{name:'Ohio State University-Main Campus',level:2})).toBeVisible();await expect(page.getByLabel('About College Scorecard average net price')).toBeVisible();await expect(page.getByText(/SAI is/)).toContainText('2,069')
  ...
  expect(requests.every(value=>/^http:\/\/127\.0\.0\.1:4173\/scorecard\/national-2026-06-10-v1\.json $/.test(value))).toBe(true)
})
...
test('compares three saved schools with shared values, sorting, removal, and reload persistence',async({page})=>{
  ...
  for(const query of ['Howard University','Massachusetts Institute of Technology','Ohio State University-Main Campus']){await page.getByLabel('Institution name').fill(query);await page.getByRole('button',{name:'Save',exact:true}).click()}
